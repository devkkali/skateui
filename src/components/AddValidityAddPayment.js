...
function AddValidityAddPayment(props) {
    ...
    const cancelClick = () => {
        ...
        setOptionSelected(props.MemberhsipTypes[0]['id']);
        let expDate = new Date();
        let startDate = new Date();
        expDate.setDate(startDate.getDate() + parseInt(props.MemberhsipTypes[0]['validity_in_day']))
        setvalidity_in_day(props.MemberhsipTypes[0]['validity_in_day']);
        settype_cost(props.MemberhsipTypes[0]['cost']);
        setEndDate(expDate);
    ...
    useEffect(() => {
        ...
        if (Details) {
            console.log("roshan is good boy", Details.membership_taken.length);
            console.log("go to hell");
            if (Details.membership_taken.length != 0) {
                setDefaultValueOfEditPayment(Details.membership_taken[Details.membership_taken.length - 1]['paid'])
            } else {
    ...
    const OnEditSubmit = () => {
        ...
        switch (props.PageType) {
            case ('membership'):
                endpoint = requests.membership_taken;
                break;
            case ('package'):
    ...
    const OnTypeChange = (e) => {
        ...
        let expDate = new Date();
        let startDate = new Date();
        const index = props.MemberhsipTypes.findIndex(item => item.id == e.target.value);
        expDate.setDate(startDate.getDate() + parseInt(props.MemberhsipTypes[index]['validity_in_day']))
        setEndDate(expDate);
        setstartDate(startDate);
        // console.log("start Date:", startDate);
        // console.log("end Date:", expDate);
        setvalidity_in_day(props.MemberhsipTypes[index]['validity_in_day']);
        settype_cost(props.MemberhsipTypes[index]['cost']);
    }
    ...
    const addValidityConfirm = () => {
        ...
        setApiLoading(true);
        setEditState("please wait");
        axios.post(requests.baseurl + endpoint + "/" + props.EditItemId, { membership_type_id: OptionSelected, paid: EditPayment }, {
            headers: {
                Accept: 'application/json',
                Authorization: 'Bearer ' + { token: localStorage.getItem('token') }.token,
                'Content-Type': 'application/json',
            ...
            .then((res) => {
                // setLoadingListDetails(false);
                // CloseSelected()
                console.log("I am here 1", props.MemberhsipTypes[0]['validity_in_day']);
                ...
                props.setLoadingListDetails(false);
                props.Reload(0);
                console.log("I am here n", props.MemberhsipTypes[0]['validity_in_day']);
                                    ...
                                    <div className="popup-3 card">
                                        ...
                                        <div className="card-body text-nowrap">
                                            <div className="mb-2 d-flex align-items-center">
                                                ...
                                                {props.MemberhsipTypes ? (
                                                    <>
                                                        <select defaultValue={props.MemberhsipTypes[0]['id']} onChange={OnTypeChange} id="card_type" className="form-select form-select-sm">
                                                            {

                                                                Object.keys(props.MemberhsipTypes).map((error, index) => (
                                                                    <option key={index} value={props.MemberhsipTypes[error]['id']} >{props.MemberhsipTypes[error]['type_name']}</option>
                                                                ))
                                            ...
                                            <div className="mb-2 d-flex align-items-center">
                                                <span className="mx-2">Charge/Hr</span>
...
export default AddValidityAddPayment